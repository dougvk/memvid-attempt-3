...
Usage:
    python file_chat.py --input-dir /path/to/documents  # Just create memory
    python file_chat.py --input-dir /path/to/documents --chat  # Create and chat
    python file_chat.py --files file1.txt file2.pdf --chunk-size 2048
...
Examples:
    ...
    python file_chat.py --input-dir ~/Documents/research
    
    # Create memory and start chat session
    python file_chat.py --input-dir ~/Documents/research --chat --provider google

    # Create memory with custom chunking for large documents
    python file_chat.py --files report.pdf --chunk-size 2048 --overlap 32

    # Use Flat index for small datasets (avoids FAISS training issues)
    python file_chat.py --files single_doc.pdf --index-type Flat

    # Load existing memory and chat (always starts chat when loading)
    python file_chat.py --load-existing output/research_memory --provider google

    # Create memory with H.265 compression
    python file_chat.py --input-dir ~/docs --codec h265
    
    # Process files in parallel with 4 workers, then chat
    python file_chat.py --input-dir ~/large-library --workers 4 --chat --provider google
"""
...
def process_single_file(file_path, chunk_size, overlap):
    ...
    try:
        if file_path.suffix.lower() == '.pdf':
            # Can't use encoder methods here, need to extract text directly
            ...
            text = ' '.join(item.get_content().decode('utf-8', errors='ignore') 
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                chunks_iter = (phrase.strip() for line in lines for phrase in line.split("  "))
                text = ' '.join(chunk for chunk in chunks_iter if chunk)
        else:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                text = f.read()
        
        # Chunk the text using memvid's chunk_text
        if text.strip():
            from memvid.utils import chunk_text
            chunks = chunk_text(text, chunk_size, overlap)
            return file_path.name, chunks, None
    except Exception as e:
...
def main():
    ...
    try:
        ...
        # Start chat session only if requested or loading existing memory
        if args.chat or args.load_existing:
            success = start_chat_session(video_path, index_path, args.provider, args.model)
            return 0 if success else 1
        else:
            print("\n✅ Memory creation complete! Use --chat to start interactive session.")
            return 0
